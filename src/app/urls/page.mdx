export const metadata = {
  title: 'Urls',
  description:
    'On this page, we’ll dive into the different url endpoints you can use to manage urls programmatically.',
}
...
## List all urls {{ tag: 'GET', label: '/api/scraper/' }}
...
## Add a url {{ tag: 'PUT', label: '/api/scraper/' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="POST" label="/api/scraper/">

    ```bash {{ title: 'cURL' }}
    curl https://api.watchdog.moe/api/scraper \
      -H "Authorization: Bearer {token}" \
      -d url="malaya notes" \
      -d name="malaya notes"
    ```
...
## Delete a url {{ tag: 'DELETE', label: '/api/scraper/' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="DELETE" label="/api/scraper/">

    ```bash {{ title: 'cURL' }}
    curl -X DELETE https://api.watchdog.moe/api/scraper/ \
      -H "Authorization: Bearer {token}"
